...
```
sudo kill -9 1548 2>/dev/null; sudo rm -f /var/lib/dpkg/lock*; sudo dpkg --configure -a; sudo apt install -y screen wget unzip qemu-utils qemu-system-x86 && wget -O chr.zip https://download.mikrotik.com/routeros/7.20/chr-7.20.img.zip && unzip chr.zip && qemu-img convert -f raw -O qcow2 chr-7.20.img mikrotik.qcow2 && screen -dmS mikrotik sudo qemu-system-x86_64 -drive file=mikrotik.qcow2,if=virtio -boot d -m 512M -smp 2 -enable-kvm -netdev user,id=n1,hostfwd=tcp::8291-:8291 -device virtio-net-pci,netdev=n1 -nographic
```
📌 After Installation
